...
// TEST
  ...
  const handleSend = async () => {
    ...
    // Call generateResponse to get AI's response
    const aiResponse = await generateResponse(chatHistory, userInput, chatHistoryIndex);
    // Append AI's response to the newChatHistory
    newChatHistory.push({ role: 'bot', content: aiResponse });

    // Update chat history state
    setChatHistory(newChatHistory);

    // Write new chat history to Firebase
    if (sessionID === "") {
        const newSessionID = generateRandomSessionID(); // Implement this function to generate a unique ID
        console.log("sessionid", newSessionID);
        setSessionID(newSessionID);
...
//     // Optionally clear chat history if starting fresh next time
//     setChatHistory([]);
